...
(define (S-27 R E x z)
  ...
  (define (s0 w)
    (t* (to-trop (|| (E x z w) (f3 w)))
        (trop #f w)))
  (define (s1)
    (define-symbolic min-w27 integer?)
    (assert (trop-lub (trop #f min-w27) s0))
    min-w27)
  (s1))
...
(define sol (verify (assert (= (rule-S R E x z) (S-27 R E x z)))))
(evaluate (rule-S R E x z) sol)
(evaluate (S-27 R E x z) sol)
(evaluate (= (rule-S R E x z) (S-27 R E x z)) sol)
