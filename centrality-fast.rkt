...
(define (interp-prog p)
  ...
  (destruct p
    ...
    [(op-sigma w x y) (sigma-3 (interp w) (interp x) (interp y))]
    [(op-delta w x y) (delta (interp w) (interp x) (interp y))]
    [p p]))
...
;; (define p (interpret interp-prog vars prog))
(define p (interpret prog))

(define M
  (synthesize
   #:forall (list E sigma D s t u v sum-i-i inv)
   ;; #:guarantee (assert (eq? (interpret interp-prog vars sketch) p))))
